...
integrateEventData <- function(filename, skipnum = 7){
  ...
  d_full <- d_raw_gaze |> 
    ...
    mutate(trial_number = cumsum(grepl("Experiment", d_raw_gaze$Event)),
           participant_group = str_split(filename, "/")[[1]][10]) |> 
...
rezeroTrials <- function(df_trial, eventname = "startContingent"){
  startScreenTimepoint <- df_trial |> filter(Event == eventname) |> pull(normalizedTimeStamp)
  df_trial <- df_trial |> 

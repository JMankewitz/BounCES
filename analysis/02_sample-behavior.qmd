...
```{r}
subj_filter <- c("BOP221r")
```

```{r fig.width = 6, fig.height = 3}

final_d_active_test <- d_active_test %>% 
  filter(!subjCode %in% subj_filter) |> 
  left_join(subj_info) %>% 
...
sample_duration_across_subj <- 
  ...
  group_by(condAOI) |> 
  summarize(N = n(), 
            mean_duration = mean(total_time_s),
          sd_duration = sd(total_time_s),
          se_duration = sd_duration/sqrt(N),
          lower = mean_duration-se_duration,
    upper = mean_duration+se_duration)
...
```{r}
samp_diff <- sample_duration_by_subj |> 
  group_by(subjCode, condAOI) |> 
  summarize(mean_samp_time = mean(total_time_ms)) |> 
  pivot_wider(values_from = mean_samp_time, names_from = condAOI) |> 
  mutate(Samp_Diff = boundary - prototype) 
```

```{r}
samp_diff |> left_join(diff_csub |> rename(subjCode = subj_code)) |> 
  left_join(subj_info) |> 
  ggplot(aes(x = mean_accuracy, y = Samp_Diff, color = age)) + 
  geom_point() + 
